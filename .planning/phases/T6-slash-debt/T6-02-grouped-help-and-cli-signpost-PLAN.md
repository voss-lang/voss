...
must_haves:
  ...
  key_links:
    ...
    - from: "voss/harness/cli.py main group docstring"
      to: "in-REPL /help"
      via: "exactly one signpost line; no epilog= kwarg; no slash-list duplication"
      pattern: "/help"
---
...
`Other` so nothing disappears), and add exactly ONE signpost line to the `main`
group docstring (cli.py:2197-2201) so `voss --help` points users at the
...
description; `voss --help` signposts once (single canonical source, no
two-place drift). This depends on T6-01 because both plans edit `cli.py`
(file-ownership serialization) and the grouped help must list the post-T6-01
`/cost` description.
...
<task type="auto">
  ...
  <read_first>
    ...
    - voss/harness/cli.py:1195 (`chat_cmd` docstring — the terse one-line style to mirror)
  ...
  <action>
    ...
    — a trailing sentence of the form `Interactive commands: run` + a backticked
    `voss chat` + `, then /help`. Match the existing terse docstring style
    (mirror `chat_cmd`'s one-line docstring at cli.py:1195). Do NOT add an
    `epilog=` kwarg to the `@click.group(...)` and do NOT duplicate the slash
    list into the CLI help (D-04 — single canonical `/help`, signpost only, no
    two-place drift). Then in `tests/harness/test_repl_slash.py`, EXTEND the
    existing `TestSlashHelp` class (do NOT remove or weaken its existing
    `test_help_lists_new_commands`) with a new test method that calls
    `_print_slash_help()` and asserts the four group headers (`Editing`,
    `Session`, `Insight`, `Control`) appear AND that `/diff`, `/resume`, `/why`
    still appear (no slash dropped by grouping). This is the SC#3 discoverability
    integration check. No fenced code in this action.
  </action>
  <verify>
    <automated>cd /Users/benjaminmarks/Projects/Voss && python -m pytest tests/harness/test_repl_slash.py::TestSlashHelp -q 2>&1 | tail -3 && test "$(python -m voss.harness --help 2>&1 | grep -c '/help')" = "1"</automated>
  </verify>
  <acceptance_criteria>
    - `python -m voss.harness --help 2>&1 | grep -c '/help'` returns exactly `1` (exactly one signpost line referencing the in-REPL `/help`).
    - `python -m voss.harness --help 2>&1 | grep -c 'voss chat'` returns `1` and that line is the same signpost line (it mentions both `voss chat` and `/help`).
    - `python -m voss.harness --help 2>&1 | grep -E '/diff|/budget|/discard|/resume'` returns NO match (no slash-list duplication into the CLI — proves there is no second slash list).
  ...
  <done>`voss --help` carries exactly one signpost line (no slash list, no `epilog=`); the new grouped-help test passes alongside the unchanged pre-existing `TestSlashHelp` test; full slash test file green.</done>
</task>
...
| CLI user → `voss --help` | Static docstring text rendered by Click; no user input crosses into it |
...
| T-T6-04 | Tampering | `voss --help` signpost docstring | accept | Static docstring, no behavior change, no state, no input. Low-value, no integrity concern; Click renders it verbatim. |
...
- `python -m voss.harness --help 2>&1 | grep -c '/help'` returns `1`; the same output has NO second slash list (grep for `/diff`/`/budget`/`/resume` returns nothing).
- `grep -n "epilog=" voss/harness/cli.py` returns no match.
</verification>

<success_criteria>
- SC#3 met: every slash appears in `/help`, grouped under named headers (Editing / Session / Insight / Control + Other) with its existing one-line description; `voss --help` signposts once at the in-REPL `/help` with no slash-list duplication.
...
MUST record the finalized group→slash membership map (the exact buckets +
whichever names landed in `Other`) so M9-03 SlashPalette and any later slash
addition can reconcile against it.
</output>
