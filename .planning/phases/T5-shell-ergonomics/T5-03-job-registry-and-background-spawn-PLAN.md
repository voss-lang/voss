...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    Session id source (D-09): `make_toolset` already has the session context available where other tools resolve it; the toolset is bound per REPL with `session_id=record.id` (cli.py:1358 / cli.py:1445 confirm `record.id` is THE session id). Thread the session_id into `shell_run_background` the same way the existing toolset captures `cwd` (closure variable in `make_toolset`). If `make_toolset` does not yet receive a session_id, add an optional `session_id: str | None = None` kwarg to `make_toolset` (additive, default None) and capture it in the closure; when None, fall back to a stable per-process default (`"_nosession"`) so unit tests that build a bare toolset still work. (T5-05 wires the real session_id through `_run_repl`'s `make_toolset` call; keep the kwarg additive so that wiring is a one-line change.)
  ...
  <done>`shell_run_background` exists, gated by shell_allowed verbatim (sandbox.py untouched), returns the bare slug, registered is_mutating=True; PID never in the return; test_background_returns_handle green.</done>
</task>
