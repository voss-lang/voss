...
4. `tests/harness/test_subagent_recursion.py` (entire file, ~56 lines) — the back-compat pinning test. Confirm the forbidden tokens it asserts absent on the `voss.harness.subagents` module: `run_subagent` has no `depth`/`max_depth` parameter; `subagents` has no `MAX_DEPTH`/`DEPTH_LIMIT`/`RECURSION_LIMIT` attribute. This plan must add NONE of those tokens to `multiagent.py` either — self-imposed via this plan's own AST/grep verify guard. NOTE: `test_subagent_recursion.py` only inspects the `voss.harness.subagents` module (it never imports `multiagent.py`); the `multiagent.py` ban is enforced by this plan's AST guard, not by that pinning test.
