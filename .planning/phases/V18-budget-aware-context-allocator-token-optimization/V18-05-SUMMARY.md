...
- **voss/harness/packing_eval.py**: `compare_runs(on, off, tolerance=0.05)` accepting row lists or runs.jsonl paths, returning a `GateResult` (dict + `.passed`); passes iff the baseline has at least one success and nonzero input tokens, `success_on >= success_off - tolerance`, and `mean_tokens_on < mean_tokens_off`. `run_packing_gate` drives the golden suite off (VOSS_NO_PACK=1, restored in finally) then on, into `out_dir/off|on`, supports profile injection through a temporary `[context]` config, and uses `auth_pref="none"` for stub runs so judge errors do not masquerade as gate evidence. Savings % (`token_reduction`) is a gate OUTPUT.
...
2. **No-savings stub run is rejected** — the short hermetic golden stub run remains useful as a driver smoke, but it is not accepted as quality/savings proof. `test_quality_preservation_gate_rejects_no_savings` pins that a failed or equal-token run cannot pass VOPT-07.
...
- `pytest tests/harness/test_packing_eval_gate.py` green (includes a real double stub-suite run through run_packing_gate, no-savings rejection, failed-baseline rejection, and profile-injection coverage).
