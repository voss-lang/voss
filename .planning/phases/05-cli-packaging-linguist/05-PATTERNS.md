...
| `language-metadata/voss.yml` | create | Local future-Linguist metadata | repo metadata -> future upstream PR prep | Linguist `languages.yml` fields | Mark as draft/local. Include `name`, `type: programming`, `extensions: [".voss"]`, `tm_scope`, `ace_mode`, `color`, `group: Python`, aliases. Do not imply upstream acceptance. |
...
| `tests/packaging/test_entrypoint.py` | create or plan-level smoke | Installed console script/package-data smoke | editable install -> `voss --help` -> resources available | `tests/parser/test_package_data.py`; `pyproject.toml` | Prefer isolated temp venv for mutation safety if automated. At minimum verify `python -m voss.cli --help`, `[project.scripts]`, and package resources. |
...
- If immediate syntax highlighting is ever prioritized, a Python fallback would be a separate product decision because it conflicts with the exact Voss declaration required by TOOL-01.
