...
voss/harness/tools.py (V21-04 adds):
  attach_memory_tools(tools, *, store, session_id, global_store=None)  # global_store kwarg is the V21 addition

voss/harness/cli.py (V21-04 wires):
  do_cmd dispatch → attach_memory_tools(..., global_store=make_global_store())  # pass-through site
...
<task type="auto">
  ...
  <done>17 RED stubs covering every VGMEM-* requirement (incl. test_do_cmd_wires_global_store dispatch pass-through), importing the real planned API, no xfail; collection green, execution red.</done>
</task>
...
<success_criteria>
- 17 RED stubs, one+ per VGMEM-* requirement (incl. do_cmd wiring pass-through), importing the real planned API
