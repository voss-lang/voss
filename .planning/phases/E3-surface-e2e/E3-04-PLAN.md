...
must_haves:
  truths:
    - "tests/eval/surfaces/ holds <=6 scenarios covering every surface: cli:do, cli:chat, cli:edit, serve (basic + permission Allow + permission Deny)"
    - "Each scenario task.toml has a surface field, a rubric, and at least one deterministic [[checks]] entry"
...
serve permission scenarios: mode="plan" forces the fs_write permission gate. auto_approve_edits=false so the
gate fires and the driver replies via /permission. The driver's permission_choice (from spec) decides Allow vs Deny.
...
<task type="auto" tdd="true">
  <name>Task 1: permission_choice field + serve Deny wiring + surface scenario dirs</name>
  ...
  <read_first>
    ...
    - voss/eval/runner.py (_drive_task serve branch from E3-03 — currently `await _drive_serve(spec, cwd)` default Allow; thread spec.permission_choice through)
  ...
  <action>
    ...
    (3) Author 6 scenario dirs under tests/eval/surfaces/ (each = task.toml + fixture/). Keep total <=6 (well under the <=10 cap). Each task.toml MUST have surface, mode, rubric, judge_inputs, and >=1 [[checks]]:
      ...
      02-chat-explain (surface="cli:chat", mode="plan"): fixture/calc.py same seed. prompt = ask to explain what add() does in one sentence (non-interactive single turn). checks: file_exists calc.py (read-only scenario; gate proves the chat turn completed and produced output — judge scores the explanation via rubric). rubric: PASS if the final mentions addition/sum.
      ...
      04-serve-write-file (surface="serve", mode="plan", auto_approve_edits=true): empty fixture (.gitkeep). prompt = write hello.py that prints "Hello, World!". checks: file_exists hello.py; file_contains hello.py "print". (Basic serve turn; auto_approve so no gate.)

      05-serve-permission-allow (surface="serve", mode="plan", auto_approve_edits=false, permission_choice="a"): empty fixture (.gitkeep). prompt = write hello.py that prints "Hello, World!" — fs_write triggers the gate, driver Allows. checks: file_exists hello.py; file_contains hello.py "print". This is the marquee D-09 proof (gate fires → Allow → file written).

      06-serve-permission-deny (surface="serve", mode="plan", auto_approve_edits=false, permission_choice="d"): empty fixture (.gitkeep). prompt = write secret.py. checks: cmd `! test -f secret.py` (Deny means the write does NOT happen) — the turn degrades without hanging; rubric: PASS if the model reports it could not write / was denied. This asserts deny-no-hang at the live level (parser-level was E3-03).
  ...
  <acceptance_criteria>
    ...
    - REQUIRED_FIELDS unchanged (permission_choice is input not output): `.venv/bin/python -m pytest tests/eval/test_voss_eval_stub.py -q` green.
    ...
    - Every scenario task.toml has >=1 [[checks]]: `grep -rl "\[\[checks\]\]" tests/eval/surfaces/*/task.toml | wc -l` == 6.
    - 05-serve-permission-allow has surface="serve", auto_approve_edits=false, permission_choice="a"; 06-serve-permission-deny has permission_choice="d".
    - No new packages installed (`git status` shows no lockfile/deps changes).
  </acceptance_criteria>
  <done>permission_choice additive field + serve Deny wiring in place; 6 surface scenarios authored covering every surface; suite loads and validates offline.</done>
</task>
...
<task type="auto">
  ...
  <done>The surfaces suite loads and dispatches correctly, proven offline; every surface routes to its driver; the Deny scenario's choice is wired.</done>
</task>
...
- load_suite(surfaces) finds 6 scenarios covering every surface; each has >=1 check
- serve Deny scenario wired with permission_choice="d"
...
<success_criteria>
- <=6 scenarios under tests/eval/surfaces/ covering cli:do/cli:chat/cli:edit/serve(basic+Allow+Deny); each with rubric + >=1 check (EVSRF-05, D-01, D-03)
- additive permission_choice field; serve Deny driven with "d"
