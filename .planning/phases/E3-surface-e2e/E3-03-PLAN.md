...
<task type="auto" tdd="true">
  ...
  <done>_drive_serve spawns/handshakes/drives a turn over SSE with stream-before-message ordering, permission reply, and guaranteed teardown; token confined to the Authorization header.</done>
</task>
...
<task type="auto" tdd="true">
  ...
  <action>
    In voss/eval/runner.py _drive_task: replace the serve not-implemented stub branch (from E3-01) with `final, crash_reason, capped = await _drive_serve(spec, cwd, permission_choice=getattr(spec, "permission_choice", "a"))` — but spec has no permission_choice field; instead read the choice from the scenario differently: the serve driver default is "a" (Allow). The Deny scenario is exercised at the unit level (Task 2 below) and via a dedicated live scenario in E3-04 that the driver reads through the existing fields. For the in-runner dispatch, call `await _drive_serve(spec, cwd)` (default Allow). Return `(record, final, crash_reason, capped)`.

    In tests/eval/test_surface_drivers.py add:

    - test_serve_stub (FAKE_TURN integration): fixture sets VOSS_SERVE_FAKE_TURN=1 in env (via monkeypatch on _live_env OR a monkeypatched os.environ that _live_env copies). Build a tmp fixture cwd. spec = TaskSpec(prompt="hello", mode="plan", rubric="...", surface="serve"). final, crash, capped = asyncio.run(_drive_serve(spec, cwd)). Assert crash is None and "echo: hello" in final (the FAKE_TURN canned final). This proves spawn → handshake → POST /session → SSE-open-before-message → final → session.idle → teardown end-to-end against the real server, no provider, no creds.
    ...
    Wave-0 fictional-API guard (MEMORY "GSD scaffold fictional API"): the synthetic-stream unit tests must call the REAL `_consume_sse` signature — import it and verify the parameter names match before relying on the fake. If `_consume_sse` is inlined rather than extracted in Task 1, adapt the unit test to whatever real helper exists; do NOT xfail around a missing helper. (If Task 1 inlined the loop, Task 1 must instead expose a small parse/reply helper this test can call — coordinate so the permission reply logic is unit-reachable.)
  ...
  <done>serve dispatch wired; FAKE_TURN integration test proves the full spawn→SSE→final plumbing offline; permission Allow/Deny proven at the parser level with deny-no-hang; token-leak threat asserted.</done>
</task>
...
| T-E3-11 | Tampering | FAKE_TURN seam activating outside tests | mitigate | VOSS_SERVE_FAKE_TURN is read only by the server (app.py:166) and set ONLY in the test fixture env; live drivers never set it; live run (E3-04) runs without it |
