...
must_haves:
  truths:
    - "conventions.has_signal(turns: Iterable[voss_runtime.memory.Turn], *, runs=None) returns True only when (≥1 user turn matches _SIGNAL_RE AND ≥2 user turns total) OR repeat-edit-same-target detected from run.changed (D-09 + Pitfall 5 quorum)"
    - "conventions.extract_conventions(history: EpisodicMemory, provider, model, *, timeout) — history typed-pinned; renders via history.render() — no dict-branch fallback"
...
<task type="auto" tdd="true">
  ...
  <behavior>
    - has_signal accepts an iterable of voss_runtime.memory.Turn instances (typically yielded from `history.turns` where `history: EpisodicMemory`).
    - has_signal([Turn(role="user", content="no use 2 spaces"), Turn(role="user", content="always them")]) -> True (matches _SIGNAL_RE + signal_a quorum: ≥2 user turns).
    - has_signal([Turn(role="user", content="summarize the repo")]) -> False (no signal hit).
  ...
  <action>
    ...
    - has_signal(turns, *, runs=None) -> bool: signature accepts an iterable of voss_runtime.memory.Turn objects (NOT dicts — typed contract). Iterate user turns: `user_turns = [t for t in turns if t.role == "user"]`. signal_a = any(_SIGNAL_RE.search(t.content) for t in user_turns). signal_b = False; if runs: from collections import Counter; changed_files = Counter(); for run in runs: changed = getattr(run, "changed", None) or (run.get("changed") if isinstance(run, dict) else []) or []; for f in changed: changed_files[f] += 1; signal_b = any(c >= 2 for c in changed_files.values()); return (signal_a and len(user_turns) >= 2) or signal_b. (Pitfall 5 quorum.) NOTE: `run` may be a RunRecord dataclass OR a dict (record.runs is list[dict] post-serialization per RESEARCH §SDK-03 context). The `runs` branch keeps the dual getattr/dict probe; the `turns` branch does not — Turn type is fixed.
    - extract_conventions(history: EpisodicMemory, provider, model: str, *, timeout=DEFAULT_EXTRACTION_TIMEOUT_SECONDS) -> list[ConventionCandidate]: async function. Signature pins history to voss_runtime.memory.EpisodicMemory — `from voss_runtime.memory import EpisodicMemory` at the top of conventions.py. Build an extraction prompt that instructs the LLM to read the conversation and emit a strict JSON array of candidate conventions, each with {statement, confidence (0..1), evidence_quote (verbatim user quote), evidence_turn_idx (int)}. The prompt should require the LLM to respond ONLY with the JSON array, no prose. Render history via `history.render()` (existing EpisodicMemory method — no manual formatting branch). Call asyncio.wait_for(provider.complete(prompt, model=model), timeout=timeout). On TimeoutError return []. On any exception, log to stderr and return []. Parse the response text as JSON via json.loads (wrap in try/except json.JSONDecodeError -> []); validate each element via ConventionCandidate.model_validate (wrap in try/except ValidationError -> []); return the validated list.
    ...
    - test_scripted_signal_session_surfaces_candidate: construct a real EpisodicMemory via `from voss_runtime.memory import EpisodicMemory; history = EpisodicMemory(capacity=40)` (capacity matches existing voss.harness.session usage); call history.add("user", "no use snake_case in Python"); history.add("assistant", "got it"); history.add("user", "always them") — produces 3 turns with 2 user-role turns (signal_a quorum) AND _SIGNAL_RE hit. Mock provider.complete via AsyncMock returning a JSON string `'[{"statement": "Use snake_case in Python", "confidence": 0.82, "evidence_quote": "no use snake_case in Python", "evidence_turn_idx": 0}]'`. Call `await extract_conventions(history, provider, "fake-model")`. Assert returned list has 1 ConventionCandidate with .statement == "Use snake_case in Python".
    ...
    - test_no_signal_skips_llm_entirely: construct `history = EpisodicMemory(capacity=40)`; call history.add("user", "summarize the repo") — 1 user turn, no signal, no quorum. Assert `has_signal(history.turns) is False`. Patch extract_conventions to fail loudly if called; call the has_signal-gated code path explicitly: if not has_signal(history.turns): result = []; assert result == [].
    - test_extraction_timeout_returns_empty: construct `history = EpisodicMemory(capacity=40)` with 2 user turns containing _SIGNAL_RE hits (so the call would fire if reached); mock provider.complete via AsyncMock with a side_effect that does `await asyncio.sleep(2.0)` then returns (longer than the override timeout). Call `await extract_conventions(history, provider, "fake-model", timeout=0.1)`. Assert returned == []; assert no exception escaped.
...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    - In do_cmd, locate the path AFTER the successful run_turn completion (around cli.py:540-547; specifically AFTER the asyncio.run(run_turn(...)) call returns and the run is recorded). The do_cmd flow already has these locals after M8-01/M8-03 land (M8-01 added voss_md_text; M8-03 added memory_store binding; M8-05 Wave 4 will add ctx.memory_store but do_cmd uses a flat local, not a ctx object). REQUIRED LOCAL NAMES (pin now; downstream plans must match):
      • `do_record` — the SessionRecord built locally in do_cmd (find via `grep -nE "record =" voss/harness/cli.py` inside the do_cmd function body; if the existing name differs, rename to `do_record` as part of this task — single-file, single-function rename).
      • `do_history` — the EpisodicMemory passed into run_turn; rename the existing local (likely `history`) to `do_history` for clarity and to mirror the rename of `record`. Verify via `grep -nE "history\s*=" inside do_cmd`.
      • `do_memory_store` — the MemoryStore bound for the one-shot run. This plan ADDS this binding (M8-03 only wired _run_repl, not do_cmd). Insert `do_memory_store = MemoryStore(cwd).bind(session_id=do_record.id)` adjacent to the M8-01 `voss_md.read_and_inject(cwd)` call in do_cmd.
      • `do_provider`, `do_model`, `do_cwd` — existing locals (just rename existing names for consistency if they differ; otherwise leave).
    - Build a `do_ctx = types.SimpleNamespace(provider=do_provider, model=do_model, cwd=do_cwd, persist_conventions_selection=None)` to satisfy the run_on_clean_exit signature (the function reads attributes via getattr, so SimpleNamespace is fine — but the NAMES are pinned here so the executor doesn't invent ad-hoc spelling). Document the pinned names in an in-source comment: `# Conventions hook locals: do_record/do_history/do_memory_store — pinned by M8-04 plan.`
    - Insert the hook: `try: conventions.run_on_clean_exit(do_ctx, history=do_history, record=do_record, memory_store=do_memory_store); except Exception as exc: click.echo(f"conventions extraction skipped: {exc}", err=True)`.
    - Place the hook AFTER the run_turn result is appended to do_record.runs (so record.runs reflects the just-completed turn for has_signal's repeat-edit detection). Place it BEFORE do_record is persisted to disk (so a session-end side effect doesn't race the write).

    (c) Extend tests/harness/test_conventions.py with one additional test test_run_on_clean_exit_smoke:
    - Use tmp_voss_repo + MemoryStore (bound as `do_memory_store`) + a SimpleNamespace `do_ctx` with .provider=AsyncMock returning a valid 1-candidate JSON array, .model="fake", .cwd=tmp_voss_repo, .persist_conventions_selection="1";
    - Build `do_history = EpisodicMemory(capacity=40)`; call do_history.add("user", "no use 2 spaces"); do_history.add("user", "always them") — 2 user turns + signal hit.
    - Build `do_record = SimpleNamespace(id="test-session", runs=[])`;
    - Monkeypatch sys.stdin.isatty -> False;
    - Call `conventions.run_on_clean_exit(do_ctx, history=do_history, record=do_record, memory_store=do_memory_store)`;
    - Assert returned == 1; assert exactly one file landed under tmp_voss_repo/.voss/memory/conventions/.
  </action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    - `grep -nE "do_record|do_history|do_memory_store" voss/harness/cli.py` returns ≥ 3 distinct local-name matches inside the do_cmd body (pinned-names invariant; M8-05/M8-06 may continue to reference these).
