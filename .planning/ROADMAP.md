...
- [ ] T5-05-voss-jobs-cli-and-active-session-PLAN.md — Wave 5: `voss jobs` table + `--json` cross-process sidecar read + AGENT_COMMANDS + `.active-session` write/remove + `--keep-logs` + explicit `reap_jobs()` on REPL exit (SHELL-05, D-04/11, A4)
