...
def _b_with_domain(domain: str):
    ...
    return ReviewerB(provider=provider, fast_model="test-fast", strong_model="test-strong")
...
class TestDomainInferred:
    ...
    def test_b_populates_valid_domain(self):
        v = _b_with_domain("code").review(_card())
        assert v.domain_inferred == "code"
        assert v.source == "B"

    def test_b_clamps_garbage_domain_to_unknown(self):
        v = _b_with_domain("banana").review(_card())
        assert v.domain_inferred == "unknown"

    def test_b_domain_always_in_allowed_set(self):
        for d in ("code", "ai", "docs", "unknown", "", "GARBAGE"):
            assert _b_with_domain(d).review(_card()).domain_inferred in _ALLOWED
