"""RED tests for SKILL-03 (trust and signature verification)."""
...
def test_tampered_manifest_refused(signed_fixture_bundle: Path, tmp_path: Path) -> None:
    ...
    try:
        ...
        pytest.fail(f"RED: missing trust or install module ({e})")
    ...
    assert not ok
    assert "tampered" in err.lower() or "signature" in err.lower()
...
def test_unknown_key_refused(signed_fixture_bundle: Path, tmp_path: Path) -> None:
    ...
    assert not ok
    assert "untrusted" in err.lower() or "unknown" in err.lower()
