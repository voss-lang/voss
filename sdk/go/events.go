...
// structs (in the generated types.gen.go). eventType() returns the member's
// canonical `type` discriminator string.
//
// This file is HAND-WRITTEN and CHECKED IN: oapi-codegen generates the structs
// and the EventEnvelope union shell but not a Go sum type or dispatcher (Go has
// no ADTs). Decode() below is the hand-written exhaustive switch; it is NOT
// regenerated by `go generate`.
type TypedEvent interface{ eventType() string }

// eventType binds each generated event struct to its discriminator string.
// Value receivers so the generated AsX() return values satisfy the interface.
