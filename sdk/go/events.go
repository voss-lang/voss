...
// TypedEvent is the sealed interface implemented by all 21 AgentEvent member
// structs (defined in the generated types.gen.go). eventType() returns the
// canonical `type` discriminator string for the member.
//
// This file is HAND-WRITTEN and CHECKED IN. oapi-codegen generates the structs
// and the EventEnvelope union shell (types.gen.go), but not a Go sum type or an
// exhaustive dispatcher — Go has no algebraic data types. Decode() below is the
// hand-written exhaustive switch that turns the generated union into a typed
// value and surfaces unknown types explicitly. It is NOT regenerated by
// `go generate` (RESEARCH Pitfall 6).
type TypedEvent interface{ eventType() string }

// eventType methods bind each generated event struct to its discriminator
// string, making it a TypedEvent. Value receivers so the values returned by the
// generated AsX() accessors satisfy the interface directly.
