"""
`rename-symbol`: deterministic, mutating, gate-enforced rename
ZERO provider calls no LLM, no agent loop. Mutating (
"""
...
def run(
    *,
    ...
    changed: list[str] = []
    for rel in files:
        # Gate self-enforcement BEFORE any mutation (landmine #3 / )
