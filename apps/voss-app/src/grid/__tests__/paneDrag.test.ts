...
describe('paneDrag controller', () => {
  ...
  it('4px move does not start drag; 6px does', () => {
    ...
    dispatch(window, 'pointermove', { clientX: 14, clientY: 10 });
    expect(drag.state()).toBeNull();

    dispatch(window, 'pointermove', { clientX: 20, clientY: 10 });
  ...
  it('Escape clears without store write', () => {
    ...
    dispatch(window, 'pointermove', { clientX: 20, clientY: 0 });
    expect(drag.state()).not.toBeNull();
  ...
  it('drop with null target mutates nothing', () => {
    ...
    dispatch(window, 'pointermove', { clientX: 20, clientY: 0 });
    dispatch(window, 'pointermove', { clientX: -50, clientY: 0 });
    dispatch(window, 'pointerup', { clientX: -50, clientY: 0 });
